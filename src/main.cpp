...
const string strMessageMagic = "Bidgram Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if (nHeight >= 50000) {
        // Subsidy is cut in half every 100,000 blocks
        nSubsidy >>= (nHeight / 100000); // Bidgram: 100K blocks
    }

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Bidgram: every 1 day
static const int64 nTargetSpacing = 60; // Bidgram: 60 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // Bidgram difficulty adjustment protocol switch
    bool fNewDifficultyProtocol = (pindexLast->nHeight+1 >= 10 || fTestNet);
    ...
    // Bidgram: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    nTime = max(pindexPrev->GetMedianTimePast()+91, GetAdjustedTime());
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal Bidgram miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Bidgram", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
		...
		//     hashMerkleRoot=6f80efd038, nTime=1509479094, nBits=1e0ffff0, nNonce=11288888, vtx=1)
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("345674710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bidgram: increase each by adding 2 to bitcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // Disconnect if not /BGRM:
        if (pfrom->strSubVer.empty() || pfrom->strSubVer.find("/BGRM:") == string::npos)
        {
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Bidgramd: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BidgramMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("BidgramMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadBidgramMiner(void* parg);

static bool fGenerateBitcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void static BidgramMiner(CWallet *pwallet)
{
    printf("BidgramMiner started\n");
    ...
    while (fGenerateBitcoins)
    {
        ...
        printf("Running BidgramMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadBidgramMiner(void* parg)
{
    ...
    try
    {
        ...
        BidgramMiner(pwallet);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadBidgramMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadBidgramMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadBidgramMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d BidgramMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadBidgramMiner, pwallet))
